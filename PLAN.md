...
- **Issuer vs. fetch address.** These are different things and the config keeps them separate. `WEB_URL` is the public URL, because Better Auth stamps it into every token as `iss` and the API rejects anything else. `JWKS_BASE_URL` is where the keys are actually fetched from — `http://web:3000`, over the compose network. Deriving both from one setting is a trap in either direction: point it inward and every request 401s on an issuer mismatch, point it outward and the fetch depends on NAT hairpinning (unreliable on Oracle) plus Caddy being up to serve a request Caddy has no part in. The same split is what makes `docker-compose.yml` work locally, where the browser's `localhost:3000` is the API container's own loopback.
